...
export const StickyScroll = ({
  ...
  return (
    <motion.div
      ...
      className="h-[30rem] overflow-y-auto flex justify-center relative space-x-32 rounded-md py-7"
      ref={ref}
    >
          ...
          {content.map((item, index) => (
            <div key={item.title + index} className="my-20">
              <motion.h2
                ...
                className="text-2xl font-bold text-pink-600 dark:text-slate-100"
              >
                {item.title}
              </motion.h2>
              <motion.p
                ...
                className="text-kg text-neutral-800 dark:text-slate-300 max-w-sm mt-10"
              >
                {item.description}
              </motion.p>
            </div>
